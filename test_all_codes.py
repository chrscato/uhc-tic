#!/usr/bin/env python3
"""Test script that processes ALL codes, not just the whitelist."""
...
def create_test_config_all_codes(payer_name: str, max_files: int = 2, max_records: int = 1000) -> ETLConfig:
    """Create a test configuration that processes ALL codes, not just whitelist."""
    ...
    test_output_dir = f"test_output_all_codes_{payer_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    
    return ETLConfig(
        payer_endpoints={payer_name: payer_endpoints[payer_name]},
        cpt_whitelist=[],  # Empty whitelist = process ALL codes
        batch_size=100,
        parallel_workers=1,
        max_files_per_payer=max_files,
        max_records_per_file=max_records,
        local_output_dir=test_output_dir,
        s3_bucket=None,
        s3_prefix="test-prefix",
        schema_version="v2.1.0-test",
        processing_version="tic-etl-all-codes-test-v1.0",
        min_completeness_pct=50.0,
        min_accuracy_score=0.7
    )

def test_all_codes(payer_name: str, max_files: int = 2, max_records: int = 1000):
    """Test processing ALL codes, not just whitelist."""
    
    print(f"=== TESTING ALL CODES FOR PAYER: {payer_name} ===")
    print(f"Max files: {max_files}")
    print(f"Max records per file: {max_records}")
    print(f"CPT Whitelist: EMPTY (processing all codes)")
    ...
    try:
        ...
        config = create_test_config_all_codes(payer_name, max_files, max_records)
...
def main():
    """Main entry point for testing all codes."""
    ...
    print("\nUsage examples:")
    print("  python test_all_codes.py bcbs_fl")
    print("  python test_all_codes.py bcbs_fl 1 500")
    
    if len(sys.argv) < 2:
        print(f"\nPlease specify a payer name. Example:")
        print(f"  python test_all_codes.py {available_payers[0]}")
        return
    ...
    test_all_codes(payer_name, max_files, max_records)
